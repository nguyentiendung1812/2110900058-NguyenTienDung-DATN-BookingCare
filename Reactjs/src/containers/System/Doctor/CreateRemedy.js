...
export default function CreateRemedy() {
  ...
  const handleResetDrugs=async ()=>{
    ...
    >
        ...
        <div className="row">
          <div className="col-12"><h5 className=""><FormattedMessage id={"admin.manage-drug.create-prescription"} /></h5></div>
        </div>
          <div className="row">
            <div className="col-8">
              <div className="row">
                          ...
                          return(
                              <div className="col-12 form-group">
                                <div className="row align-item-center text-center">
                                    <div className="col-4"> <input readonly type="text" value={drug.name} className="form-control" placeholder="" /></div>
                                    <div className="col-auto">
                                        <select className="form-control" onChange={(event)=>handleOnchangeUnitDrug(event,drug.id)} value={drug.unit}>
                                    ...
                                    <div className="col-1"> <input onChange={(event)=>handleOnchangeAmountDrug(event,drug.id)} type="text" value={drug.amount} className="form-control" /></div>
                                    <div className="col-4"> <input onChange={(event)=>handleOnchangeDescriptionUsageDrug(event,drug.id)} type="text" value={drug.desciption} className="form-control" placeholder={language=="en" ? "Enter desription usage" : "Nhập hướng dẫn sử dụng" } /></div>
                                    <div className="col-1 d-flex align-items-center"><i onClick={()=>removeDrugFromTheList(drug.id)} className="fas fa-trash pointer text-red"></i></div>
                                </div>
                              </div>
                          );
                        })
            ...
            <div className="col-4">
                    <div className="row">
                        <div className="col-12"><FormattedMessage id={"admin.manage-drug.drug-list"} />
                        ...
                        value={queryDrug}
                         ...
                         <div className="mt-10">
                          ...
                          <ul className="list-group mt-10" style={{overflowY: "scroll", maxHeight: "400px"}}>
                            {
                              listFilterDrugs.map((drug)=>{
                                return(
                                  <li onClick={()=>handlePushDrugToList(drug)} className="list-group-item list-group-item-action d-flex justify-content-between align-items-center pointer"><span>{drug.name}</span> <i className="text-primary fas fa-plus-circle"></i></li>
                                )
                              })
                            }
                          </ul>
          ...
          <button  onClick={() => handleCreateRemedyImage()} type="button" className="btn btn-primary"><FormattedMessage id={"admin.manage-drug.btn-create"} /></button>
